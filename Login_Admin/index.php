<?php
    ...
    //Iniciando sessão para o idioma
    session_start();
    //Verificação
    if(isset($_POST["entrar"])) {
		$usuario        = mysqli_escape_string($conexao, $_POST["usuario"]);
	    $senha          = mysqli_escape_string($conexao, $_POST["senha"]);
	    $verificar      = mysqli_query($conexao, "SELECT * FROM admin_bj WHERE usuario = '$usuario' AND senha = '$senha'");
	    if (mysqli_num_rows($verificar) <= 0 ) {
	    	$_SESSION['erroLogin'] = "Informações de login inválidas!";
	    } else {
            session_start();
            $result = mysqli_fetch_array($verificar);
	        $_SESSION['Admin_Logado'] = $result['id_admin'];
...
<head>
    ...
    <title>Painel Administrativo | Bom Jobs Recrutamento</title>
    <style>
        #login { width: 40%; }
...
<body>
    ...
    <main>
        ...
        <h2 class="text-primeira">Gestão Interna - Bom Jobs</h2>
