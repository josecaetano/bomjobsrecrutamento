<?php
    ...
    if(isset($_POST["entrar"])) {
	    ...
	    $verificar      = mysqli_query($conexao, "SELECT * FROM contratante_bj WHERE usuario = '$usuario' AND senha = '$senha'");
	    if (mysqli_num_rows($verificar) <= 0 ) {
	    	$_SESSION['erroLogin'] = "Informações de login inválidas!";
	    } else {
            session_start();
            $contratante = mysqli_fetch_array($verificar);
	        $_SESSION['Contratante_Logado'] = $contratante['id_contratante'];
	        header('Location: ../Painel_Contratante/');
	    }
	}
?>
<!doctype html>
...
<head>
    ...
    <title>Login do Contratante | Bom Jobs Recrutamento</title>
    <style>
        #login { width: 40%; }
...
<body>
    ...
    <main>
        ...
        <h2 class="text-primeira">Contratante - Bom Jobs Recrutamento</h2>
            <form action="" method="POST">
                <div class="mb-3">
                    <label for="inputUsuario" class="form-label">Nome de Usuário</label>
                    <input type="text" class="form-control" id="inputUsuario" name="usuario" placeholder="O seu nome de usuário" required>
                </div>
                <div class="mb-3">
                    <label for="inputSenha" class="form-label">Senha</label>
                    <input type="password" class="form-control" id="inputSenha" name="senha" placeholder="A sua senha" required>                
                </div>
